# This is a little demonstration about the Linear Search vs. Binary Search
# Assumes Python 3
...
def main():
    print(
        ...
        "and I will try to guess it using Linear Search!\n"
    )
    ...
    if get_yn("Are you ready? (y/n): "):
        ...
        if got_it:
            ...
            print("Something smells wrong here...\n"
                  "Let's me change my search strategy...\n"
                  "I will try to guess it using Binary Search this time in no more than 7 tries!"
                  )
            if get_yn("Are you ready? (y/n): "):
                lo, hi = 1, 100
                got_it = False
                for attempt in range(1, 8):
                    guess = (lo + hi) // 2  # binary search
                    print("I guess {}!".format(guess))
                    res = get_one_of("Was this [L]ow, [H]igh, or [C]orrect? ", {"l", "h", "c"})
                    if res == "l":
                        lo = guess + 1
